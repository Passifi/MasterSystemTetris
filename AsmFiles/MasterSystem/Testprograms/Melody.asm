...
Programstart:
	...
	ld sp, &dff0
	ld a,0
	ld (NoteIndex),a ;initialise NoteIndex to 0
	;init screen
	ld hl,VdpInitData
...
mainloop:
	...
	inc a 
	cp 40
	jp nz, Continue ;(Timer at 40?) if not continue other stuff 
	ld hl,MusicData 
	ld a,(NoteIndex)
	or a
	jp z, PlayIt ; plays current note? 
	ld b,a 
	ld a,0
raiseToIndex: 
	inc hl 
...
PlayIt: 	
	call PlayNote
	ld a,0 ;reset timer after playing note
	ld (Timer),a 
	ld a,(lastA) 
